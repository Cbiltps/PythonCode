def split_file(file_content: str):
    ...
    if catalogue_match:
        ...
        attachments_text = file_content[
            catalogue_match.start() :
        ]  # 附件部分（包含"附件"标记）
    else:
        # 未找到附件标记，全部视为正文
    ...
    for chap_idx, chap in enumerate(chapter_iter):
        ...
        # 确定章节内容范围（到下一章节或正文结束）
        chap_end = (
            chapter_iter[chap_idx + 1].start()
            if chap_idx + 1 < len(chapter_iter)
            else len(main_text)
        )
    ...
    if attachments_text:  # 如果存在附件部分
        ...
        attachment_iter = list(
            re.finditer(attachment_pattern, attachments_text)
        )  # 查找所有附件标记
        ...
        for idx, att in enumerate(attachment_iter):
            ...
            next_start = (
                attachment_iter[idx + 1].start()
                if idx + 1 < len(attachment_iter)
                else len(attachments_text)
            )
