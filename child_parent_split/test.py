def split_file(file_content: str):
    ...
    pre_content_lines = []  # 收集第一个一级标题之前的内容
    ...
    for line in lines:
        ...
        if current_section is not None:
            ...
            if stripped_content or (
                current_section["all_lines"] and current_section["all_lines"][-1] != ""
            ):
                current_section["all_lines"].append(
                    stripped_content if stripped_content else ""
                )
        else:
            # 第一个一级标题之前的内容
            stripped_content = line.strip()
            if stripped_content or (
                pre_content_lines and pre_content_lines[-1] != ""
            ):
                pre_content_lines.append(
                    stripped_content if stripped_content else ""
                )
        ...
        for line in section["all_lines"]:
            ...
            if current_subsection is not None:
                ...
                if line or (
                    ...
                    and current_subsection["lines"][-1] != ""
                ):
                    current_subsection["lines"].append(line if line else "")
            else:
                # 直属内容（还没遇到子标题，或者子标题已经结束）
                if line or (
                    section["direct_lines"] and section["direct_lines"][-1] != ""
                ):
    ...
    # 第三步：按顺序生成chunks
    # 处理第一个一级标题之前的内容（如果有）
    if pre_content_lines:
        pre_content = "\n".join(pre_content_lines).strip()
        if pre_content:
            parent_id_counter += 1
            pre_section_id = parent_id_counter
            # 创建一个特殊的父文档用于存储前置内容
            chunks.append(
                {
                    "id": pre_section_id,
